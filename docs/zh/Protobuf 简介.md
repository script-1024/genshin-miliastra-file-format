...
- C/C++
  ...
  int size_of_varint_buffer(uint64_t value) {
      int size = 1;
      while (value >= 0x80) { // 若仍剩有数据
        value >>= 7; // 每个字节编码 7 个有效位
        size++;
      }
      return size;
  }
...
- C#
  ...
  static byte[] ToVarint(ulong value) {
      ...
      for (int i = 0; i < size; i++) {
          ...
          value >>= 7;
          if (value != 0) data |= 0x80; // 若存在剩余数据，标记最高位
          result[i] = data;
      }
      return result;
  }
...
2. 某些标签可能缺失，可能乱序出现，可能重复出现（例如存储了一系列对象的列表），因此解析器必须具备灵活读取标签的能力，不能依赖从文件读到的字节序列。
    > 注：尽管根据观察，原神应该都是按标签编号顺序写入文件的，但你不可以假定它们的顺序，因为第三方程序未必会遵守此规则！

3. 字段缺失时，理应返回该类型的默认值。
    > 注：原神有时会在某些值为零时直接省略不写入文件中，这点需要特别注意！
  
4. 当拥有相同 ID 的标签在当前作用域内重复出现时
   1. 若字段为基本类型，由新值覆写旧值。
   2. 若字段为列表类型，追加元素到末尾。
   3. 若字段为复合结构，按照 **新值覆写旧值**、**实值取代空值**、**空值不改变旧值** 的规则进行合并。

5. 标签自身的类型为 Varint，使用最低 3 位表示类型，其余有效位表示编号，格式为：`(id << 3) | type`。

6. 标签的编号必须是正整数。

7.  标签可用的种类如下，原神使用它们来表示特定类型：
   
    | ID | 名称 | 用途 |
    |:--:|:----|:-----|
    |0|VARINT|整数、枚举、布尔值|
    |1|FIXED64|双精度浮点数、定长64位整数（较少见）|
    |2|LENGTH|字符串、复合结构、列表、字典|
    |3|GROUP_START|（不使用）|
    |4|GROUP_END|（不使用）|
